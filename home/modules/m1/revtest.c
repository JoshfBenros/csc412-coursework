...
int main(int argc, char** argv) {
  ...
  if (argc == 1){
    ...
    */ 
    ...
    // Parse the first argument as num_elements
    int num_elements = atoi(argv[1]);
    
    // Calculate actual count of remaining arguments
    int actual_count = argc - 2;  
    
    // Check if they match
    if (num_elements != actual_count) {
    ...
    // Populate array from argv starting at argv[2]
