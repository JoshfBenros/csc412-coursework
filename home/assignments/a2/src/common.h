...
// Global player name used by render_game_over()
// Must be declared here so game_over.c can access it
...
typedef struct snake {
    size_t* positions;   // positions[0] is head, positions[length-1] is tail
    size_t length;
    size_t capacity;
    int dx;              // current movement direction x component
    int dy;              // current movement direction y component
    int initialized;     // 0 until snake state is initialized from board
} snake_t;
